...
public class TestLineChartView extends View {
    ...
    private void drawTitle(Canvas canvas) {
        ...
        canvas.drawText("近七天收益趋势", getTextWidth(titleSize, "----"), textHeight,
                paint);
    }
...
//        }
    ...
    private void drawX(Canvas canvas) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            Path path = new Path();
            path.moveTo(marginLeft + i * widthInterval - getTextWidth(xSize, dates.get(i)) / 2, height + textHeight * 7/ 2+60);// 只用于移动移动画笔。
    ...
    private void drawY(Canvas canvas) {
        ...
        for (int i = 0; i < numbers.size(); i++) {
            canvas.drawText(String.valueOf(numbers.get(i)), width, i * heightInterval+ height, paint);
        }
